...
function FeaturedProjectCard({ repo, index }: FeaturedProjectCardProps) {
  ...
  return (
    ...
    >
      ...
      >
        <span className="text-4xl text-white/50 font-mono">{repo.name.charAt(0).toUpperCase()}</span>
      </div>
          ...
          {repo.topics && repo.topics.length > 0 && (
            <div className="flex flex-wrap gap-1.5 mb-4">
              {repo.topics.map((topic) => (
                <Tag key={topic} size="sm">{topic}</Tag>
              ))}
            </div>
          )}
        </div>
